...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (message) => {
    ...
    try {
      ...
      // Si la demande concerne l'historique des messages
      if (parsed.get_chat_history) {
        const request = parsed.get_chat_history;
        grpcClient.GetChatHistory(request, (error, response) => {
          if (error) {
            ws.send(JSON.stringify({ error: error.message }));
          } else {
            ws.send(JSON.stringify({ chat_messages: response.chat_messages }));
          }
        });
      } else {
